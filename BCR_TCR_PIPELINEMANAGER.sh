...
if [[ "$STAGE" -eq 4 ]]; then 
	...
	JOB_ID4=$(qsub -pe shmem 16 -q long.qc -t 1 -e COMMANDLOGS/${SAMPLES_FILE_POST}/CAT/ -N BCR_TCR_PIPELINE_CAT -o COMMANDLOGS/${SAMPLES_FILE_POST}/CAT/ -terse -hold_jid ${JOB_ID1} cat_IMGT_files.sh ${DEPENDANCIES} ${SAMPLES_FILE_POST} ${BATCH_FILE} ${CODE_DIRECTORY} YES)
...
if [[ "$STAGE" -eq 6 ]]; then 
	...
	# RUN CAT
	JOB_ID=$(qsub -pe shmem 10 -q long.qc -t 1 -e COMMANDLOGS/${SAMPLES_FILE_POST}/CAT/ -N BCR_TCR_PIPELINE_CAT -o COMMANDLOGS/${SAMPLES_FILE_POST}/CAT/ -terse BCR_TCR_Wrapper_2.sh NO ${SAMPLES_FILE_POST} ${BATCH_FILE} ${CODE_DIRECTORY} ${IMGT_MUTATION})
