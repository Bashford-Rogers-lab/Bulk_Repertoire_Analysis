...
def Get_paired_reads_overlapping(file1, file2, outfile,gene,paired,id,method):
  (rc)= Init_rc()
  (seqs1,seqs2)=Get_sequences(file1)
...
def Get_sequences(file):
  ...
  for header,sequence in fasta_iterator(fh):
    header = header.split()
    f = header[1][0]
    header = header[0].replace(":","").split()[0].replace("-","")
    header = header.split("#")[0].split("/")[0]
    if(f=="1"):seqs1[header]=sequence
    elif(f=="2"):seqs2[header]=sequence
  fh.close()
  return(seqs1, seqs2)
...
def Filter_IgJ_genes(Trim1, Trim2, refj,primer_file,ref_const,primer_tag_file_count,blastall,refv):
  mode ="WITHIN" 
  fh=open(Trim2,"w")
  fh.close()
  if(1==1):
    if(mode == "WITHIN"):
      ...
      for header,sequence in fasta_iterator(fh):
        ...
        c = c+1
        if(batch>=batch_size):
          Blast_match_J(out, seqs, Trim1, Trim2, refj,e_value,blastall,refv)
          out,batch,seqs = '',0,{}
          print "BATCH"
      fh.close()
      if(len(out)>2):
...
def Blast_match_J(out, seqs, Trim1, Trim2, refj,e_value, blastall,refv):
  fh=open(Trim1+"_blast_J","w")
  fh.write(out)
  fh.close()
...
if(command_source.count("2")!=0):
  if(gene.count("IG")!=0):
    ...
    Filter_IgJ_genes(Trim1, Trim2, refj,primer_file,ref_const,primer_tag_file_count,blastall,refv)
