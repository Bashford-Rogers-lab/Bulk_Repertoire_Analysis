make_matrices13 <- function(file=file, ids_all=ids_all){
	info = file.info(file)
	if(info$size != 0) {
		...
		p <- p[, c("X.sample", "isotype", "mean_CDR3_R_K_residues")]
		colnames(p) <- c("Sample", "isotype", "mean_CDR3_R_K_residues_CHARGE")
