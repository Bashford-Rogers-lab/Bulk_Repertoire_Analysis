...
## Code to calculate within module correlation networks to identify higher order repertoire structure
## Lauren Overend
...
plot_correlation_within <- function(eigenvectors, outputdir, type_receptor){
	...
	##----------------------------------------------------------------------------
	#### PART 1 CORRELATION USING ALL DATA violates INDEPENDENCE
	...
	corrplot(rval, type="upper", p.mat=pval, insig="label_sig", tl.pos="td", sig.level=0.05, title =paste0("Within Module Correlations BH correction: ", type_receptor), method = "circle", diag = F, order = 'original', tl.cex = 0.7, mar=c(0,0,2,0), pch.cex = 2)
	dev.off()
	...
	plot(ggraph(graph_cors) +ggraph::geom_edge_link(aes(edge_alpha = abs(r), edge_width = abs(r), edge_color = r)) +guides(edge_alpha = "none", edge_width = "none")+ 
	geom_node_point(color = "black", size = 3) + geom_node_text(aes(label = name), repel = TRUE) +theme_graph(base_family="sans") +
	labs(title = paste0(type_receptor, " Correlation Network (BH adj p<0.05 & R>0.3)"))+
	scale_edge_color_gradient2(low = "blue", high = "red", mid = "white"))
	dev.off() 

	##----------------------------------------------------------------------------
	################################################################################
	## PART 2 CALCULATE CORRELATION  FOR EACH DAY !!!
	##----------------------------------------------------------------------------
	## DAY 1
	...
	par( mfrow= c(1,3) )
	corrplot(rval, type="upper", p.mat=pval, insig="label_sig", tl.pos="td", sig.level=0.05, title =paste0("Within Module Correlations BH correction Day 1: ", type_receptor), method = "circle", diag = F, order = 'original', tl.cex = 0.7, mar=c(0,0,2,0), pch.cex = 2)
	## DAY 3
	...
	rval3 <- rval
	pval3 <- pval
	corrplot(rval, type="upper", p.mat=pval, insig="label_sig", tl.pos="td", sig.level=0.05, title =paste0("Within Module Correlations BH correction Day 3: ", type_receptor), method = "circle", diag = F, order = 'original', tl.cex = 0.7, mar=c(0,0,2,0), pch.cex = 2)
	## DAY 5
	...
	rval5 <- rval
	pval5 <- pval
	corrplot(rval, type="upper", p.mat=pval, insig="label_sig", tl.pos="td", sig.level=0.05, title =paste0("Within Module Correlations BH correction Day 5: ", type_receptor), method = "circle", diag = F, order = 'original', tl.cex = 0.7, mar=c(0,0,2,0), pch.cex = 2)
	dev.off()

	#----------------------------------------------------------------------------
	################################################################################
	## PART 3 CALCULATE CORRELATION NETWORKS FOR EACH DAY !!!
	### Day 1 
	...
	rval2 <- rval2 %>%filter(abs(r) > 0.3)
	graph_cors <- rval2 %>%graph_from_data_frame(directed = FALSE)
	p1 <- ggraph(graph_cors) + geom_edge_link(aes(edge_alpha = abs(r), edge_width = abs(r), color = r)) +guides(edge_alpha = "none", edge_width = "none") +scale_edge_color_gradient2(low = "red", high = "blue", mid = "white") + geom_node_point(color = "black", size = 3) +geom_node_text(aes(label = name), repel = TRUE) + theme_graph(base_family="sans") +labs(title = paste0(type_receptor, " modules Day 1 (BH adj p<0.05 & R>0.3)"))
	...
	#----------------------------------------------------------------------------
	################################################################################
	## PART 4 CALCULATE CORRELATION NETWORKS using RMCORR!!!!!!!
	...
	corrplot(to_plot, type="upper", p.mat=m, insig="label_sig", tl.pos="td", sig.level=0.05, title =paste0("Within Module Correlations RMCORR: ", type_receptor), method = "circle", diag = F, order = 'original', tl.cex = 0.7, mar=c(0,0,2,0), pch.cex = 2)
	corrplot(to_plot, type="upper", p.mat=madj, insig="label_sig", tl.pos="td", sig.level=0.05, title =paste0("Within Module Correlations RMCORR BH adj: ", type_receptor), method = "circle", diag = F, order = 'original', tl.cex = 0.7, mar=c(0,0,2,0), pch.cex = 2)
	dev.off()
	...
	#----------------------------------------------------------------------------
	################################################################################
	## PART 5 PUT ALL THE PLOTS TOGETHER
