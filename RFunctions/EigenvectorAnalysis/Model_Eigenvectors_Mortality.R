...
correlate_eigenvectors_outcome <- function(eigenvectors, metadata, outputdir, type_receptor, run_all){
	...
	for(i in 2:(length(colnames(eigenvectors))-16)){
		...
		p1 <- interactions::interact_plot(xmdl, pred = DAY, modx = Mortality2, partial.residuals = TRUE, interval = TRUE, y.label="Residual\nModule Score", main= colnames(eigenvectors)[i], legend.main="Mortality")
		myplots[[(i-1)]] <- p1
	...
	pdf(paste0(outputdir, "/Mortality_AdjModel_Residual_Plot.pdf"),width=dims, height=dims)
	...
	if(interaction_pres =="YES"){
		...
		p_hd_extended$Type <- "ADDITIVE MODEL"
		...
		p_hd_extended$Type <- str_wrap(p_hd_extended$Type, width = 12)
		...
		pdf(paste0(outputdir, "/Mortality_SimpleEffects_Statistics_", annot, ".pdf"), width=14, height=13)
