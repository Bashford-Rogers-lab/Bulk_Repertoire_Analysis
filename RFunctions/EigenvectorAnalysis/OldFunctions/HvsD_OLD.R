...
correlate_eigenvectors_hvd <- function(eigenvectors, metadata, outputdir, type_receptor, metahealth){
	...
	pdf(paste0(outputdir,"/HealthvsDisease_Scatter.pdf"), width=13, height=13)
	...
	p_values_hd <- c()
	vif_all <- c()
	cols <- as.numeric(length(2:(length(colnames(eigenvectors))-7)))
	myplots <- list()
	behaviour <- c()
	...
	for(i in 2:(length(colnames(eigenvectors))-7)){
		...
		model_formula <- formula(paste0(colnames(module)[1], "~", "DAY", "*","DISEASE + Age + Sex + Comorbidities.Charlson_Index"))
		...
		p1 <- interactions::interact_plot(xmdl, pred = DAY, modx = DISEASE, partial.residuals = TRUE, interval = TRUE )
		myplots[[(i-1)]] <- p1
	...
	colnames(p_values_hd) <- c("Module", "TIMEPOINT", "DISEASE.SEPSIS", "AGE", "SEX.MALE", "COMORBIDITIES:CI", "TIMEPOINTxSEPSIS", "F_TEST") 	
	
	#--------------------------------------------------------
	### Lets quickly plot the model VIF scores (is colinearity a problem)	
	pdf(paste0(outputdir,"/VIF_Scatter.pdf"), width=5, height=5)
	...
	plot(ggplot(vif_all_g, aes(x=Effect, y=VIF))+geom_col(alpha=0.5) +theme_bw()+ylab("Module Score")+xlab("Day")+ggtitle(paste0("Model VIFF Scores")) +facet_wrap(~V1)+geom_hline(yintercept=5, col="red") + xlab("Model Effect") + ylab("VIFF")+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
	dev.off()
	#--------------------------------------------------------
	...
	pdf(paste0(outputdir, "/HealthvsDisease_Residual.pdf"), width=(dims+5), height=dims)
	...
	p_hd <- gather(p_values_hd, "Effect", "p_value", TIMEPOINT:F_TEST, factor_key=TRUE)
	...
	write.table(p_hd, paste0(outputdir, "/Summary/INTERACTION_ANALYSIS_hvsd.txt"), sep="\t")
	
	## Lets take out the F test scores
	#p_hd_d  <- p_hd[p_hd$Effect =="F_TEST",]
	## Now lets do a BH correction
	#p_hd <- p_hd[p_hd$Effect !="F_TEST",]
	...
	p_hd$Type <- "Effect"
	p_hd$Type[p_hd$Effect =="F_TEST"] <- "F Test"
	p_hd$Effect <- as.character(p_hd$Effect)
    p_hd$Effect[p_hd$Effect =="F_TEST"] <- "MODEL"
	p_hd$Effect <- factor(p_hd$Effect, levels=c("TIMEPOINT", "DISEASE.SEPSIS", "TIMEPOINTxSEPSIS", "AGE", "SEX.MALE", "COMORBIDITIES:CI", "MODEL"))
	write.table(p_hd, paste0(outputdir, "/Summary/INTERACTION_ANALYSIS_hvsd_padj.txt"), sep="\t")
	...
	pdf(paste0(outputdir, "/HvsD_InteractionModel.pdf"), width=11, height=5)
	p1 <- ggplot(p_hd, aes(x=Module, y=Effect)) +  geom_tile(aes(fill = as.numeric(p_value), alpha=ifelse(p_value <= 0.05, 1, 0.9)), colour="lightblue")+theme_classic()+ scale_fill_continuous(type = "viridis")+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ggtitle(paste0(type_receptor, " Interaction Model, p<0.05")) + labs(fill="P") + guides(alpha="none")+xlab("Module") +ylab("Model Coefficients")+
		geom_tile(aes(color=factor(BHsig, c("sig", "ns"))), fill = '#00000000', size = 0.5) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type),  scales="free_y", space='free_y')+ geom_point(aes(x=Module, y=Effect, shape = Direction), fill="white", col="black", size=2.5) + scale_shape_manual(values=c(24,25), labels = c('Increase','Decrease'), na.translate=FALSE, drop=FALSE) +labs(shape="Direction of Effect")
	plot(plot_grid(p1, px, ncol=2, align="h", axis="lbt", rel_widths=c(2.5,1), labels="AUTO"))
	p2 <- ggplot(p_hd, aes(x=Module, y=Effect)) +  geom_tile(aes(fill = as.numeric(p_value), alpha=ifelse(p_value <= 0.05, 1, 0.9)), colour="lightblue")+theme_classic()+ scale_fill_continuous(type = "viridis", na.value="white", limits=c(0, p_use))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ggtitle(paste0(type_receptor, " Interaction Model, p<0.05")) + labs(fill="P") + guides(alpha="none")+xlab("Module") +ylab("Model Coefficients")+
			geom_tile(aes(color=factor(BHsig, c("sig", "ns"))), fill = '#00000000', size = 0.5) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type),  scales="free_y", space='free_y')+ geom_point(aes(x=Module, y=Effect, shape = Direction), fill="white", col="black", size=2.5) + scale_shape_manual(values=c(24,25), labels = c('Increase','Decrease'), na.translate=FALSE, drop=FALSE) +labs(shape="Direction of Effect")
	plot(plot_grid(p2, px, ncol=2, align="h", axis="lbt", rel_widths=c(2.5,1), labels="AUTO"))	
	p3 <- ggplot(p_hd, aes(x=Module, y=Effect)) +  geom_tile(aes(fill = as.numeric(p_value), alpha=ifelse(p_value <= 0.05, 1, 0.9)), colour="lightblue")+theme_classic()+ scale_fill_continuous(type = "viridis", na.value="white", limits=c(0, 0.1))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ggtitle(paste0(type_receptor, " Interaction Model, p<0.05")) + labs(fill="P") + guides(alpha="none")+xlab("Module") +ylab("Model Coefficients")+
			geom_tile(aes(color=factor(BHsig, c("sig", "ns"))), fill = '#00000000', size = 0.5) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type),  scales="free_y", space='free_y')+ geom_point(aes(x=Module, y=Effect, shape = Direction), fill="white", col="black", size=2.5) + scale_shape_manual(values=c(24,25), labels = c('Increase','Decrease'), na.translate=FALSE, drop=FALSE) +labs(shape="Direction of Effect")
	...
	if(interaction_pres =="YES"){
		...
		for(i in 1:length(modules_interaction)){
			...
			model_formula <- formula(paste0(colnames(module)[1], "~", "DAY+Age + Sex + Comorbidities.Charlson_Index")) #+Barcode
			xmdl = lm(model_formula, module)
			...
			p_vals <- p_vals[2:4]
			x1  <- summary(xmdl)
			f <- pf(x1$fstatistic[1],x1$fstatistic[2],x1$fstatistic[3],lower.tail=FALSE)
			p_vals <- c(p_vals, NA, f)
			## Get estimates 
			...
			colnames(p_all) <- c("MODULE", "DISEASE", "DAY", "AGE", "SEX", "COMORBIDITIES:CI", "F_TEST")
			...
			colnames(row_use) <- c("MODULE", "DAY", "DISEASE", "AGE", "SEX", "COMORBIDITIES:CI", "F_TEST")
		...
		p_hd_extended_timepoint <- gather(p_hd_extended_timepoint, "Effect", "p_value", DAY:F_TEST, factor_key=TRUE)
		...
		p_hd_extended_timepoint$Difference <- NA
		p_hd_extended_timepoint$Normalised <- NA

		## Now we want to say whether its higher at the later timepoint or not 
		## This is for the timepoint analysis!!!!
		for(i in 1:length(p_hd_extended_timepoint[,1])){
			row_used <- p_hd_extended_timepoint[i,]
			if(row_used$Effect %like% "DAY"){
				print(i)
				...
				p_hd_extended_timepoint$Difference[i] <- diff_means
				p_hd_extended_timepoint$Normalised[i] <- diff_means_normalised 
			}
		}
		
		#----------------------------------------------------
		## Now we want to say whether its higher in sepsis or health 
		## This is for the HVS diseas analysis	
		p_hd_extended_hvsd <- data.frame(p_hd_extended_hvsd)
		p_hd_extended_hvsd <- gather(p_hd_extended_hvsd, "Effect", "p_value", DISEASE:F_TEST, factor_key=TRUE)
		...
		p_hd_extended_hvsd$Difference <- NA
		p_hd_extended_hvsd$Normalised <- NA
		for(i in 1:length(p_hd_extended_hvsd[,1])){	
			row_used <- p_hd_extended_hvsd[i,]
			if(row_used$Effect=="DISEASE"){
				module_use <- as.character(row_used[,1])
				data_use <- eigenvectors[,c(module_use, "sample", "DAY", "DISEASE")]
				if(row_used$DAY %like% "Day1"){
					DAY <- "Day1"
				}
				if(row_used$DAY %like% "Day3"){
					DAY <- "Day3"
				}
				if(row_used$DAY %like% "Day5"){
					DAY <- "Day5"
				}
				...
				diff_means_normalised <- (diff_means/means$Mean[means$DISEASE=="HEALTH"])*100
				p_hd_extended_hvsd$Difference[i] <- diff_means
				p_hd_extended_hvsd$Normalised[i] <- diff_means_normalised 
			}
		}
		#----------------------------------------------------	
		#----------------------------------------------------	
		...
		# Timepoint
		p_hd_extended_timepoint$Difference <- as.numeric(p_hd_extended_timepoint$Difference)
		p_hd_extended_timepoint$Normalised <- as.numeric(p_hd_extended_timepoint$Normalised)
		p_hd_extended_timepoint$High <- NA
		p_hd_extended_timepoint$High[p_hd_extended_timepoint$Difference> 0] <- "High"
		p_hd_extended_timepoint$High[p_hd_extended_timepoint$Difference< 0] <- "Low"
		p_hd_extended_timepoint$DISEASE[p_hd_extended_timepoint$DISEASE %like% "Health"] <- "HEALTH"
		p_hd_extended_timepoint$DISEASE[p_hd_extended_timepoint$DISEASE %like% "Sepsis"] <- "SEPSIS"
		p_hd_extended_timepoint$High <- factor(p_hd_extended_timepoint$High, levels=c("High", "Low", NA))
		
		p_hd_extended_timepoint$Type <- "Effect"
		p_hd_extended_timepoint$Type[p_hd_extended_timepoint$Effect =="F_TEST"] <- "F Test"
		p_hd_extended_timepoint$Effect <- as.character(p_hd_extended_timepoint$Effect)
		p_hd_extended_timepoint$Effect[p_hd_extended_timepoint$Effect =="F_TEST"] <- "MODEL"
		p_hd_extended_timepoint$Effect[p_hd_extended_timepoint$Effect =="COMORBIDITIES.CI"] <- "COMORBIDITIES:CI"
		p_hd_extended_timepoint$Effect <- factor(p_hd_extended_timepoint$Effect, levels=c("DAY", "AGE", "SEX", "COMORBIDITIES:CI", "MODEL"))
	
	
		# HvsD
		p_hd_extended_hvsd$Difference <- as.numeric(p_hd_extended_hvsd$Difference)
		p_hd_extended_hvsd$Normalised <- as.numeric(p_hd_extended_hvsd$Normalised)
		p_hd_extended_hvsd$High <- NA
		p_hd_extended_hvsd$High[p_hd_extended_hvsd$Difference> 0] <- "High"
		p_hd_extended_hvsd$High[p_hd_extended_hvsd$Difference< 0] <- "Low"
		p_hd_extended_hvsd$High <- factor(p_hd_extended_hvsd$High, levels=c("High", "Low", NA))
		
		p_hd_extended_hvsd$Type <- "Effect"
		p_hd_extended_hvsd$Type[p_hd_extended_hvsd$Effect =="F_TEST"] <- "F Test"
		p_hd_extended_hvsd$Effect <- as.character(p_hd_extended_hvsd$Effect)
		p_hd_extended_hvsd$Effect[p_hd_extended_hvsd$Effect =="F_TEST"] <- "MODEL"
		p_hd_extended_hvsd$Effect[p_hd_extended_hvsd$Effect =="COMORBIDITIES.CI"] <- "COMORBIDITIES:CI"
		p_hd_extended_hvsd$Effect <- factor(p_hd_extended_hvsd$Effect, levels=c("DISEASE", "AGE", "SEX", "COMORBIDITIES:CI", "MODEL"))
		...
		## Lets save the resutlts
		write.table(p_hd_extended_timepoint, paste0(outputdir, "/Summary/Timepoint_ANALYSIS_hvsd.txt"), sep="\t")
		write.table(p_hd_extended_hvsd, paste0(outputdir, "/Summary/HvsD_ANALYSIS_hvsd.txt"), sep="\t")
		...
		p_hd_extended_timepoint$High[p_hd_extended_timepoint$p_value >= 0.05] <- NA
		p_hd_extended_timepoint$High <- factor(p_hd_extended_timepoint$High, levels=c(NA, "High", "Low"))
		
		p_hd_extended_hvsd$High[p_hd_extended_hvsd$p_value >= 0.05] <- NA
		p_hd_extended_hvsd$DAY <- gsub("Day", "Day ", p_hd_extended_hvsd$DAY)
		p_hd_extended_hvsd$High <- factor(p_hd_extended_hvsd$High, levels=c(NA, "High", "Low"))
		...
		pdf(paste0(outputdir,"/HealthvsSepsis_PDistributions.pdf"), width=8, height=6)
		plot(ggplot(p_hd_extended_hvsd) + geom_histogram(aes(x = p_value, fill=DAY),alpha=0.5,breaks = seq(0, 1, 0.05), color = "black") +scale_y_continuous(expand = expansion(c(0, 0.05))) +
		  facet_wrap(vars(Effect)) + theme_bw(base_size = 12) + geom_vline(xintercept=0.05, col="red") + xlab("p value") +ylab("Count")+ggtitle(paste0("Distribution of P values for LR HvsS Model ", type_receptor)) +labs(fill="Model"))
		plot(ggplot(p_hd_extended_timepoint) + geom_histogram(aes(x = p_value, fill=DISEASE),alpha=0.5,breaks = seq(0, 1, 0.05), color = "black") +scale_y_continuous(expand = expansion(c(0, 0.05))) +
		  facet_wrap(vars(Effect)) + theme_bw(base_size = 12) + geom_vline(xintercept=0.05, col="red") + xlab("p value") +ylab("Count")+ggtitle(paste0("Distribution of P values for LR Timepoint Models ", type_receptor)) +labs(fill="Model"))
		dev.off()  
	
		######################################################
		## Now we want to do the P value plots 
		## This is just so we get colours for the y axis
		# Doesnt work if you use the facet grid as it follows the same order!
		#orders <- unique(p_hd_extended_hvsd[, c("Effect")])
		#a <- ifelse(orders %like% "DISEASE", "red", "black")		
		#orders <- unique(p_hd_extended_timepoint[, c("Effect")])
		#a1 <- ifelse(orders %like% "DAY", "red", "black")
	
		##........................................
		### Finally plot the results of each layer of the model!
		pdf(paste0(outputdir,"/HealthvsDisease.pdf"), width=11, height=5)
		p1 <- ggplot(p_hd_extended_hvsd, aes(x=MODULE, y=Effect)) + geom_tile(aes(fill = as.numeric(p_value), alpha=ifelse(p_value < p_use, 1, 0.9))) + 
		scale_fill_continuous(type = "viridis")+ geom_point(data = p_hd_extended_hvsd, aes(x=MODULE, y=Effect, shape = High), fill="red", col="red", size=2) + 
		scale_shape_manual(values=c(24,25), labels = c('Increase rel. Health','Decrease rel. Health'), na.translate=FALSE, drop=FALSE) +
		theme_classic() +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
		labs(fill="P", shape="Dynamics") + guides(alpha="none")+xlab("Module") +ylab("Effect") +ggtitle("Health Vs Disease Model") +
		geom_tile(aes(color=factor(BHsig, c("sig", "ns"))), fill = '#00000000', size = 1) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type), cols=vars(DAY),  scales="free_y", space='free_y')
		
		p2 <- ggplot(p_hd_extended_timepoint, aes(x=MODULE, y=Effect)) + geom_tile(aes( fill = as.numeric(p_value), alpha=ifelse(p_value < p_use, 1, 0.9))) + 
		scale_fill_continuous(type = "viridis")+ geom_point(data = p_hd_extended_timepoint, aes(x=MODULE, y=Effect, shape = High), fill="red", col="red", size=2) + 
		scale_shape_manual(values=c(24,25), labels = c('Increase over Time','Decrease over Time'), na.translate=FALSE, drop=FALSE) +
		theme_classic() +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
		labs(fill="P", shape="Dynamics") + guides(alpha="none")+xlab("Module") +ylab("Effect") +ggtitle("Time Model")+
		geom_tile(aes(color=BHsig), fill = '#00000000', size = 1) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type), cols=vars(DISEASE), scales="free_y", space='free_y')
		plot(plot_grid(p1, p2, labels="AUTO", align="h", axis="lbt" ))
		
		### with scaled expression
		p1 <- ggplot(p_hd_extended_hvsd, aes(x=MODULE, y=Effect)) + geom_tile(aes( fill = as.numeric(p_value), alpha=ifelse(p_value < p_use, 1, 0.9)), colour="lightgrey") + 
		scale_fill_continuous(type = "viridis", na.value="white", limits=c(0, p_use))+ geom_point(data = p_hd_extended_hvsd, aes(x=MODULE, y=Effect, shape = High), fill="red", col="red", size=2) + 
		scale_shape_manual(values=c(24,25), labels = c('Increase rel. Health','Decrease rel. Health'), na.translate=FALSE, drop=FALSE) +
		theme_classic() +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
		labs(fill="P", shape="Dynamics") + guides(alpha="none")+xlab("Module") +ylab("Effect") +ggtitle("Health Vs Disease Model")+
		geom_tile(aes(color=factor(BHsig, c("sig", "ns"))), fill = '#00000000', size = 1) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type), cols=vars(DAY), scales="free_y", space='free_y')

		p2 <- ggplot(p_hd_extended_timepoint, aes(x=MODULE, y=Effect)) + geom_tile(aes(fill = as.numeric(p_value), alpha=ifelse(p_value < p_use, 1, 0.9)), colour="lightgrey") + 
		scale_fill_continuous(type = "viridis", na.value="white", limits=c(0, p_use))+ geom_point(data = p_hd_extended_timepoint, aes(x=MODULE, y=Effect, shape = High), fill="red", col="red", size=2) + 
		scale_shape_manual(values=c(24,25), labels = c('Increase over Time','Decrease over Time'), na.translate=FALSE, drop=FALSE) +
		theme_classic() +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
		labs(fill="P", shape="Dynamics") + guides(alpha="none")+xlab("Module") +ylab("Effect") +ggtitle("Time Model")+
		geom_tile(aes(color=factor(BHsig, c("sig", "ns"))), fill = '#00000000', size = 1) + scale_color_manual(name = "BH Adj P", values = c("red", '#00000000'), drop = FALSE)+facet_grid(rows=vars(Type),  cols=vars(DISEASE), scales="free_y", space='free_y')
