...
correlate_eigenvectors_outcome <- function(eigenvectors, metadata, outputdir, type_receptor, run_all){
	...
	pdf(paste0(outputdir,"/Mortality_Module_Scatter.pdf"), width=10, height=10)
	...
	for(i in 2:(length(colnames(eigenvectors))-16)){
		...
		module <- eigenvectors[,c(module, "sample", "DAY", "Mortality2", "Age", "Sex", "Comorbidities.Charlson_Age","Comorbidities.Charlson_Index", "Barcode")]
		...
		model_formula <- formula(paste0(colnames(module)[1], "~", "DAY*Mortality2 + Age + Sex  + Comorbidities.Charlson_Index+(1|Barcode)"))
	...
	pdf(paste0(outputdir,"/Mortality_VIF_Score.pdf"), width=5, height=5)
	vif_all <- data.frame(vif_all)
	vif_all_g <- gather(vif_all, "Effect", "VIF", DAY:DAY.Mortality2, factor_key=TRUE)
	...
	pdf(paste0(outputdir, "/Mortality_AdjModel_Residual_Plot.pdf"),width=(dims+5), height=dims)
	...
	write.table(p_hd, paste0(outputdir, "/Summary/Mortality_INTERACTION_ANALYSIS_SUMMARY.txt"), sep="\t")
	...
	pdf(paste0(outputdir, "/Mortality_InteractionModel_Statistics.pdf"), width=12, height=6)
	...
	if(interaction_pres =="YES"){
		...
		for(i in 1:length(modules_interaction)){
			...
			module <- eigenvectors[,c(module, "Barcode", "sample", "DAY", "Mortality2", "Age", "Sex", "Comorbidities.Charlson_Age","Comorbidities.Charlson_Index")]
			modulex <- module 
			...
			## Day 1 MORTALITY
			module <- modules_interaction[i]
			module <- eigenvectors[,c(module, "Barcode", "sample", "DAY", "DISEASE", "Mortality2", "Age", "Sex", "Comorbidities.Charlson_Age","Comorbidities.Charlson_Index")]
		...
		write.table(p_hd_extended_timepoint, paste0(outputdir, "/Summary/Mortality_INTERACTION_ANALYSIS_timepoint_", annot, ".txt"), sep="\t")
		## Lets remove the direction for ns effect
		...
		write.table(p_hd_extended, paste0(outputdir, "/Summary/Mortality_INTERACTION_ANALYSIS_hvsd_", annot, ".txt"), sep="\t")
		## Lets remove the direction for ns effect
		...
		pdf(paste0(outputdir,"/Mortality_SimpleEffects_PDistributions.pdf"), width=8, height=4)
