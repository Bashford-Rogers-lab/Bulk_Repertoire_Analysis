...
optimise_imputing_per_row <- function(final_matrix, iso_type, na_freq, outputdir, type_use){
	...
	df_verif <- t(data.frame(apply(df, 1, function(x) {prodNA(data.frame(x), na_freq)})))
	df_verif <- data.frame(df_verif)
	colnames(df_verif) <- colnames(df)
	rownames(df_verif) <- rownames(df)

	## Plot the patterns of missingness in the data 
	pdf(paste0(outputdir, "Plots/Imputation/Missingness_patterns_across_sample", na_freq, ".pdf"), width=60, height=40)
	...
	pdf(paste0(outputdir, "Plots/Imputation/Missingness_values_across_sample", na_freq, ".pdf"), width=widthx, height=heightx)
	...
	write.table(nrmse_combined, (paste0(outputdir, "Summary/nrmsescores_", type_use, "_", iso_type, "_", na_freq, "_per_sample.txt")), sep="\t", row.names=FALSE)
	...
	pdf(paste0(outputdir, "Plots/Imputation/Imputation1_", type_use, "_", iso_type, "_", na_freq, "_per_sample.pdf"), width=13, height=10)
	...
	pdf(paste0(outputdir, "Plots/Imputation/Imputation2_Correlation_", type_use, "_",  iso_type, "_", na_freq, "_per_sample.pdf"), width=15, height=15)	
	...
	pdf(paste0(outputdir, "Plots/Imputation/Imputation2_", type_use, "_", iso_type, "_", na_freq, "_per_sample.pdf"), width=80, height=20)
