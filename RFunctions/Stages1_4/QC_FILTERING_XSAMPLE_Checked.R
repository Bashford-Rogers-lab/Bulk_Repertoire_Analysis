...
visualise_filtering_bcr <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, cluster_nodes = 5){
	...
	pdf(paste0(path_to_outputdir,'/Plots/PERSAMPLE_FILTERING_QC_OVERALL_', Run_name, '.pdf'), width=widthx, height=heightx)
	...
	plot(plot_grid(p1,p3, p2, p4, ncol=1))
	dev.off()	
	pdf(paste0(path_to_outputdir,'/Plots/PERSAMPLE_FILTERING_QC_PERSTAGE_', Run_name, '.pdf'), width=widthx, height=10)
	...
	write.table(Filtering_Results, paste0(path_to_outputdir, "/Summary/PERSAMPLE_FILTERING_", Run_name, ".txt"), sep="\t")		
}
...
visualise_filtering_bcr_layouts <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, path_to_layout = path_to_layout, cluster_nodes = 5){
	...
	pdf(paste0(path_to_outputdir,'/Plots/PERSAMPLE_FILTERING_QC_OVERALL.pdf'), width=widthx, height=heightx)
	...
	pdf(paste0(path_to_outputdir,'/Plots/PERSAMPLE_FILTERING_QC_PERSTAGE.pdf'), width=widthx, height=10)
	...
	write.table(Filtering_Results, paste0(path_to_outputdir, "/Summary/PERSAMPLE_FILTERING.txt"), sep="\t")		
}
