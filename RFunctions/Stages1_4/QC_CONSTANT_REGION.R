...
visualise_constant_region_bcr <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, plot_dir=plot_dir, cluster_nodes = 5){
	library(tidyverse)
	library(ggplot2)
	library(foreach)
	library(doParallel)
	...
	write.table(Constant_Results, paste0(path_to_outputdir, "/Summary/PERSAMPLE_CONSTANT_USAGE_QC.txt"), sep="\t")		
}


visualise_constant_region_bcr_layouts <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, path_to_layout=path_to_layout, plot_dir=plot_dir, cluster_nodes = 5){
	library(tidyverse)
	library(ggplot2)
	library(foreach)
	library(doParallel)
