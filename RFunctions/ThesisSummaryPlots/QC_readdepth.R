	...
	library(data.table)
#--------------------------------------------------------------------------------------------------------------------------------------
visualise_filtering_bcr_layouts_neat <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, path_to_layout = path_to_layout, plot_dir=plot_dir, stat_dir=stat_dir, cluster_nodes = 5){
	...
	Filtering_Results$DISEASE <-  "SEPSIS"
	Filtering_Results$DISEASE[Filtering_Results$Sample  %like% "HV"] <-  "HEALTH"
	Filtering_Results$DISEASE[Filtering_Results$Sample  %like% "POSITIVE"] <-  "TECHNICAL"
	pdf(paste0(plot_dir,'/Filtering_QC_', run_name, '.pdf'), width=13, height=7)
	p1 <- ggplot(Filtering_Results, aes(x = Library, y=N_UniqueBCRS, fill=Lane)) + geom_boxplot(alpha=0.5) +facet_grid(~Lane,  scales="free_x", space="free_x") + theme_bw() + xlab("Sequencing Library") + ylab("Number of  Unique VDJs") +guides(fill="none")+ geom_hline(yintercept=(mean(Filtering_Results$N_UniqueBCRS) + sd(Filtering_Results$N_UniqueBCRS)), col="purple", linetype='dotted')+geom_hline(yintercept=(mean(Filtering_Results$N_UniqueBCRS) - sd(Filtering_Results$N_UniqueBCRS)), col="purple", linetype='dotted') +geom_hline(yintercept=mean(Filtering_Results$N_UniqueBCRS), col="red", linetype='dotted')+geom_hline(yintercept=1500, col="green")
	p2 <- ggplot(Filtering_Results, aes(x = Library, y=PercentagePassedFiltering, fill=Lane)) + geom_boxplot(alpha=0.5)  +facet_grid(~Lane,  scales="free_x", space="free_x") + theme_bw() + xlab("Sequencing Library") + ylab("% of Reads Passed Filtering") +guides(fill="none")+ geom_hline(yintercept=(mean(Filtering_Results$PercentagePassedFiltering) + sd(Filtering_Results$PercentagePassedFiltering)), col="purple", linetype='dotted')+geom_hline(yintercept=(mean(Filtering_Results$PercentagePassedFiltering) - sd(Filtering_Results$PercentagePassedFiltering)), col="purple", linetype='dotted') +geom_hline(yintercept=mean(Filtering_Results$PercentagePassedFiltering), col="red", linetype='dotted')
	p3 <- ggplot(Filtering_Results, aes(x = DISEASE, y=N_UniqueBCRS, fill=DISEASE)) + geom_boxplot(alpha=0.5)+ theme_bw() + xlab("DISEASE") + ylab("Number of  Unique VDJs") +guides(fill="none")+ geom_hline(yintercept=(mean(Filtering_Results$N_UniqueBCRS) + sd(Filtering_Results$N_UniqueBCRS)), col="purple", linetype='dotted')+geom_hline(yintercept=(mean(Filtering_Results$N_UniqueBCRS) - sd(Filtering_Results$N_UniqueBCRS)), col="purple", linetype='dotted') +geom_hline(yintercept=mean(Filtering_Results$N_UniqueBCRS), col="red", linetype='dotted')+geom_hline(yintercept=1500, col="green")
