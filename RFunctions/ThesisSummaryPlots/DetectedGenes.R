...
#path_to_outputdir <- '/gpfs2/well/immune-rep/shared/MISEQ/SEPSIS_COMPLETE/BCR'
setwd('/gpfs3/well/immune-rep/shared/CODE/BCR_TCR_PROCESSING_PIPELINE')

visualise_vj_QC_neat <- function(path_to_outputdir = path_to_outputdir, cluster_nodes = 5, chain=chain){
	library(tidyverse)
	library(ggplot2)
	library(foreach)
	library(doParallel)
	...
	if(any(frequency_variable_genes_id$V_Gene %like% "IGH")){
		...
		Known_V_genes$Family <- str_split_fixed(Known_V_genes$Vgene, "-", 2)[,1]
		Known_V_genes$Family <- str_split_fixed(Known_V_genes$Family, "\\/", 2)[,1]
		
		pdf(paste0(path_to_outputdir, '/THESIS_PLOTS/V_Gene_Usage_Detection.pdf'), width=12, height=5)
	...
	if(any(frequency_variable_genes_id$V_Gene %like% "TR")){
		...
		pdf(paste0(path_to_outputdir, '/THESIS_PLOTS/V_Gene_Usage_Detection.pdf'), width=20, height=10)
