...
compare_technicals_neat <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, technical_replicates_file=technical_replicates_file, plot_dir=plot_dir, path_to_layout = path_to_layout, stat_dir=stat_dir, cluster_nodes = 5){
	...
	percentage <- frequency_variable_genes_J %>% group_by(Sample, J_gene) %>% summarise(cnt = count) %>% mutate(percent = cnt / sum(cnt)*100, totalcnt =sum(cnt))
	percentage2 <- percentage  %>% group_by(J_gene)  %>% summarize(Meanpercentage = round(mean(percent, na.rm=TRUE),2), SDpercentage = round(sd(percent, na.rm=TRUE),2), , NSDpercentage=round(sd(percent, na.rm=TRUE)/mean(percent, na.rm=TRUE),2))
	colnames(percentage2)[1] <- "Gene"
	percentagex <- frequency_variable_genes_family %>% group_by(Sample, V_family) %>% summarise(cnt = count) %>% mutate(percent = cnt / sum(cnt)*100, totalcnt =sum(cnt))
	percentage2x <- percentagex  %>% group_by(V_family)  %>% summarize(Meanpercentage = round(mean(percent, na.rm=TRUE),2), SDpercentage = round(sd(percent, na.rm=TRUE),2), , NSDpercentage=round(sd(percent, na.rm=TRUE)/mean(percent, na.rm=TRUE),2))
	...
	plot(p1)
	dev.off()
	stats <- Constant_Results_subset %>% arrange(Lane) %>% group_by(Lane, gene)  %>% summarize(Meanpercentage = round(mean(percentage, na.rm=TRUE),2), SDpercentage = round(sd(percentage, na.rm=TRUE),2), NSDpercentage=round(sd(percentage, na.rm=TRUE)/mean(percentage, na.rm=TRUE),2))
	colnames(stats) <- c("Lane", "Isotype", "Mean % of Repertoire", "SD % of Repertoire", "NSD % of Repertoire")
	stats2 <- Constant_Results_subset %>% arrange(Lane) %>% group_by(gene)  %>% summarize(Meanpercentage = round(mean(percentage, na.rm=TRUE),2), SDpercentage = round(sd(percentage, na.rm=TRUE),2), , NSDpercentage=round(sd(percentage, na.rm=TRUE)/mean(percentage, na.rm=TRUE),2))
	...
	stasx <- Constant_Results_subsetx %>% summarize(Meanpercentage = round(mean(totalreads, na.rm=TRUE),2), SDpercentage = round(sd(totalreads, na.rm=TRUE),2), NSDpercentage = round(sd(totalreads, na.rm=TRUE)/mean(totalreads, na.rm=TRUE),2))
...
compare_technicals_neat_tcr <- function(path_to_outputdir = path_to_outputdir, run_name = run_name, technical_replicates_file=technical_replicates_file, plot_dir=plot_dir, path_to_layout = path_to_layout, stat_dir=stat_dir, chain=chain, cluster_nodes = 5){
	...
	percentage2 <- percentage  %>% group_by(J_gene)  %>% summarize(Meanpercentage = round(mean(percent, na.rm=TRUE),2), SDpercentage = round(sd(percent, na.rm=TRUE),2), , NSDpercentage=round(sd(percent, na.rm=TRUE)/mean(percent, na.rm=TRUE),2))
	colnames(percentage2)[1] <- "Gene"
	percentagex <- frequency_variable_genes_family %>% group_by(Sample, V_Gene) %>% summarise(cnt = count) %>% mutate(percent = cnt / sum(cnt)*100, totalcnt =sum(cnt))
	percentage2x <- percentagex  %>% group_by(V_Gene)  %>% summarize(Meanpercentage = round(mean(percent, na.rm=TRUE),2), SDpercentage = round(sd(percent, na.rm=TRUE),2), , NSDpercentage=round(sd(percent, na.rm=TRUE)/mean(percent, na.rm=TRUE),2))
	colnames(percentage2x)[1] <- "Gene"
	results <- rbind(percentage2, percentage2x)
	colnames(results) <- c("Gene", "Mean % of Repertoire", "SD % of Repertoire", "NSD % of Repertoire")
	pdf(paste0(plot_dir,'/Filtering_Vgene_stats.pdf'), width=10, height=10)
