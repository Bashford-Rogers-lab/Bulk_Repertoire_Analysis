...
summary_imputation <- function(path_to_outputdir, plot_dir, stat_dir){
	...
	meaners <- imputation_methods %>% group_by(method,NAseedfrequency)  %>% summarize(MeanRMSE = round(mean(rsme),2))
	...
	means_group <- imputed_vs_non %>% group_by(variable,Type)  %>% summarize(Mean = mean(value, na.rm=TRUE)) %>% spread(Type, Mean)
	...
	d <- na_count %>% group_by(variable) %>% rowwise() %>% mutate(isna= sum(is.na(value)))
