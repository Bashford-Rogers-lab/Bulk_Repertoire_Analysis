...
get_best_genes <- function(cohort1_file, cohort2_file, eigenvectors_file, outputdir, cohort_assignment, module_use){
		...
		all_cors$Type[all_cors$GeneSymbol %like% "^IGH" | all_cors$GeneSymbol %like% "^IGL" | all_cors$GeneSymbol %like% "^IGK"] <- "AIR"
		...
		#################################################################################################
		if(any(!is.na(all_labs$gene))& length(!is.na(all_labs$gene))>1){
		...
		#--------------------------------------------------------------------------------------------
		## Gene ontology analysyis 
		## First using gprofiler2
}
