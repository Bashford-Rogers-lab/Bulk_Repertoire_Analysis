...
cyber_sort <- '/gpfs2/well/immune-rep/shared/MISEQ/LEO_GAinS_RNASEQ_2023/Yuxin_PROTEOMIC_PROCESSED/REPERTOIRE_SAMPLES_IMMUNOGLOBULIN.txt'
...
correlate_protein <- function(eigenvectors, cyber_sort, outputdir, metadata, metahealth){
		...
		pdf(paste0(outputdir, "/Protein_VIF_Score.pdf"), width=5, height=5)
		...
		pdf(paste0(outputdir, "/Protein_HvsD_Model.pdf"), width=10, height=8)
		...
		write.table(p_hd, paste0(outputdir, "Protein_HvsD_Model.txt"), sep="\t")
		...
		pdf(paste0(outputdir, "/Protein_HvsD_InteractionModel_Statistics.pdf"), width=13, height=6)
	...
	if(interaction_pres =="YES"){
		...
		for(i in 1:length(modules_interaction)){
			...
			pdf(paste0(plot_dir_tukey2,"/Protein_HvsD_TukeyTest", colnames(eigenvectors)[i], ".pdf"), width=8, height=7)  
			par( mfrow= c(2,3) )
			...
			dev.off()
			write.table(tukey_all, paste0(plot_dir_tukey2, "/HvsD_Protein_TukeyTest_Statistics_",modulename, ".txt"), sep="\t")
		...
		write.table(p_hd_extended_timepoint, paste0(outputdir, "/HvsD_Protein_ANALYSIS_timepoint_", annot, ".txt"), sep="\t")
		## Lets remove the direction for ns effect
		...
		write.table(p_hd_extended_hvsd, paste0(outputdir, "/HvsD_Protein_ANALYSIS_hvsd_", annot, ".txt"), sep="\t")
		## Lets remove the direction for ns effect
		...
		pdf(paste0(outputdir,"/HvsD_Protein_SimpleEffects_PDistributions_", annot, ".pdf"), width=8, height=4)
		...
		pdf(paste0(outputdir, "/HvsD_Protein_SimpleEffects_Statistics_", annot, ".pdf"), width=10, height=10)
